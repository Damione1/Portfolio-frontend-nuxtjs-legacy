...
export default {
  ...
  generate: {
    routes: () => {
      const routes = []
      axios.get(`${process.env.API_URL}/public/blogpost/${process.env.userId}`).then((res) => {
        res.data.forEach((blog) => {
          routes.push('/blog/' + blog.slug)
        })
      })

      axios.get(`${process.env.API_URL}/public/project/${process.env.userId}`).then((res) => {
        res.data.forEach((portfolio) => {
          routes.push('/project/' + portfolio.slug)
        })
      })
    ...
    exclude: [
      /^\/backend/ // path starts with /admin
    ]
